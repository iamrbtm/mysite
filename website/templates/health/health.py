...
def doctors():
    ...
    facilities = db.session.query(Facility).filter(Facility.userid == flask_login.current_user.id).order_by(Facility.name).all()
...
def surgeries():
    ...
    doctors = db.session.query(Doctor).filter(Doctor.userid == flask_login.current_user.id).order_by(Doctor.fulldisplayname).all()
...
def hospital():
    ...
    doctors = db.session.query(Doctor).filter(Doctor.userid == flask_login.current_user.id).filter(Doctor.lastname != "").order_by(Doctor.lastname, Doctor.firstname).all()
    facilities = db.session.query(Facility).filter(Facility.userid == flask_login.current_user.id).filter(Facility.type == "Hosptial").order_by(Facility.name).all()
...
@login_required
# IDEA: Make it so the user can know when the medication is due to be refilled.
# IDEA: Create a 8.5 x 11 report in pdf to print out. (made report medlist.html.  Need to get it to print with pdfkit)
def medications():
    if request.method == 'POST':
    ...
    inactivemeds = db.session.query(Medications).filter(Medications.userid == flask_login.current_user.id).filter(Medications.active == False).order_by(Medications.next_refill, Medications.name).all()

    context = {'user':User, 'facilities':facilities, 'doctors':doctors, 'pharmacy':pharmacy, 'medications':medications, 'inactivemeds':inactivemeds}
    return render_template("health/medications.html", **context)
...
def medupdate(id):
    if request.method == 'POST':
        ...
        med.active = eval(request.form.get('active'))
        db.session.commit()
...
def makeCard(id): 
    doctorinfo = db.session.query(Doctor.fulldisplayname, Doctor.address, Doctor.city, Doctor.state, Doctor.zip, Doctor.phone, Doctor.email, Facility.name.label('company')).filter(Doctor.id == id).join(Facility,Facility.id == Doctor.facilityfk).first()
...
@login_required
def doctors_update(id):
    if request.method == 'POST':
        formdata = request.form.to_dict()
        print(formdata)
        update_doctor(id, **formdata)
        return redirect(url_for('health.doctors'))
        
    doctors = db.session.query(Doctor).filter(Doctor.id == id).first()
    facilities = db.session.query(Facility).filter(Facility.userid == flask_login.current_user.id).filter(or_(Facility.type == 'Clinic', Facility.type == 'Hosptial', Facility.type == 'Pharmacy')).all()
...
def medlistPrint():
    allergies =  db.session.query(Allergies).filter(Allergies.userid == flask_login.current_user.id).all()
    medications = db.session.query(Medications).filter(Medications.userid == flask_login.current_user.id).filter(Medications.active == True).order_by(Medications.reason_for_taking,Medications.name).all()
    doctors = db.session.query(Doctor).filter(Doctor.userid == flask_login.current_user.id).order_by(Doctor.fulldisplayname).all()
    return render_template("health/medicationlist.html", user=User, medications=medications, doctors=doctors, allergies=allergies)
...
@health.route("/hospital/hosplist", methods=['GET'])
@login_required
def hospitallist():
    stays = db.session.query(Hosptial).filter(Hosptial.userid == flask_login.current_user.id).order_by(desc(Hosptial.datestart)).all()
    LOS = []
    ...
    facilities = db.session.query(Facility).filter(Facility.userid == flask_login.current_user.id).filter(Facility.type == "Hosptial").order_by(Facility.type, Facility.name).all()
    return render_template("health/hospitallist.html", user=User, stays=stays, doctors=doctors, facilities=facilities, los=LOS)

@health.route("/surgeries/surgerylist", methods=['GET', 'POST'])
@login_required
def surgerylist():
    if request.method == 'POST':
    ...
    doctors = db.session.query(Doctor).filter(Doctor.userid == flask_login.current_user.id).order_by(Doctor.fulldisplayname).all()
    facilities = db.session.query(Facility).filter(Facility.userid == flask_login.current_user.id).filter(Facility.type != "Pharmacy").order_by(Facility.name).all()
    return render_template("health/surgerylist.html", user=User, facilities=facilities, doctors=doctors, surgeries=surgeries)

@health.route("/surgeries/<id>", methods=['GET', 'POST'])
@login_required
def surgeries_update(id):
    if request.method == "POST":
        formdata = request.form.to_dict()
        update_surgeries(id, **formdata)
        return redirect(url_for('health.surgeries'))
    
    surgeries = db.session.query(Surgeries).filter(Surgeries.userid == flask_login.current_user.id).filter_by(id=id).first()
    doctors = db.session.query(Doctor).filter(Doctor.userid == flask_login.current_user.id).order_by(Doctor.lastname, Doctor.firstname).all()
    facilities = db.session.query(Facility).filter(Facility.userid == flask_login.current_user.id).filter(Facility.type != "Pharmacy").order_by(Facility.name).all()
