...
def goals():
    if request.method == "POST":
        newGoal = Goals(
            ...
            datestart=datetime.datetime.strptime(
                request.form.get("datestart"), "%Y-%m-%d"
            ),
    ...
    completedTasks = (
        db.session.query(Tasks.goalfk, func.count(Tasks.id).label("count"))
        .filter(Tasks.checked == True)
        .group_by(Tasks.goalfk)
        .all()
    )
    allTasks = (
        db.session.query(Tasks.goalfk, func.count(Tasks.id).label("count"))
...
def goal_single(id):
    ...
    completedTasks = (
        db.session.query(Tasks.item)
        .filter(Tasks.userid == current_user.id)
        .filter(Tasks.goalfk == id)
        .filter(Tasks.checked == True)
        .count()
    )
    allTasks = (
    ...
    goals = (
        ...
        .filter(Goals.userid == current_user.id)
        .filter(Goals.id == id)
        .all()
    )
    tasks = (
        ...
        .filter(Tasks.userid == current_user.id)
        .filter(Tasks.goalfk == id)
        .order_by(Tasks.checked, Tasks.duedate, Tasks.item)
        .all()
    )
    return render_template(
        "productivity/goals_single.html",
        user=User,
        goals=goals,
        tasks=tasks,
        percentcomplete=percentcomplete,
...
def projects():
    if request.method == "POST":
        ...
        nextrev = datetime.datetime.strptime(
            request.form.get("last_reviewed"), "%Y-%m-%d"
        ) + timedelta(days=when)
        newProj = Projects(
            name=request.form.get("name"),
            pictureurl=request.form.get("pictureurl"),
    ...
    completedTasks = (
        db.session.query(Tasks.project, func.count(Tasks.id).label("count"))
        .filter(Tasks.checked == True)
        .group_by(Tasks.project)
        .all()
    )
    allTasks = (
        db.session.query(Tasks.project, func.count(Tasks.id).label("count"))
...
def tasks(page_num):
    ...
    projects = (
        ...
        .filter(Projects.userid == current_user.id)
    ...
    incomplete = (
        db.session.query(
            ...
            Projects.status.label("projectstatus"),
            Tasks.checked,
            Tasks.duedate,
            Tasks.goalfk,
            Tasks.id,
        ...
        .filter(Tasks.userid == current_user.id)
        .filter(Tasks.checked == 0)
        .order_by(Tasks.duedate, Tasks.project, Tasks.item)
    ...
    complete = (
        ...
        .join(Projects, Projects.id == Tasks.project)
        .filter(Tasks.userid == current_user.id)
        .filter(Tasks.checked == 1)
        .order_by(Tasks.project, Tasks.duedate, Tasks.item)
...
def project_single(id):
    if request.method == "POST":
        if request.form["reviewupdate"] == "Review Complete":
            ...
            udproject.next_review = datetime.datetime.strptime(
                request.form.get("date_update"), "%Y-%m-%d"
            ) + timedelta(days=int(udproject.when_review))
            udproject.last_reviewed = datetime.datetime.strptime(
                request.form.get("date_update"), "%Y-%m-%d"
            )
            db.session.commit()
            ...
            newtask = Tasks(
                ...
                duedate=datetime.datetime.strptime(
                    request.form.get("dueDate"), "%Y-%m-%d"
                ),
                goalfk=None,
            )
            db.session.add(newtask)
    ...
    projects = (
        db.session.query(
            ...
            Projects.name,
            Projects.status,
            Projects.last_reviewed,
            Projects.when_review,
            Projects.pictureurl,
            Projects.userid,
        )
        .filter(Projects.userid == current_user.id)
    ...
    return render_template(
        "productivity/projects_single.html",
        user=User,
        projects=projects,
        tasks=tasks,
        percentcomplete=percentcomplete,
...
@login_required
def importNotion():
    from website.templates.productivity.process_productivity import (
        get_tasks,
        getNextTasks,
        parsejsonMakeTask,
        notionidupdate,
...
def taskupdate(taskid):
    if request.method == "POST":
        ...
        updatetask.duedate = datetime.datetime.strptime(
            request.form.get("duedate"), "%Y-%m-%d"
        )
