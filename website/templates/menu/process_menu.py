...
def move_plan(id, date):
    newid = (
        db.session.query(Planner.id, Planner.wknum)
        .filter(Planner.date == datetime.strptime(date, "%Y-%m-%d"))
        .first()
    )
...
def get_holidays(year=datetime.now().year):
    ...
    url = (
        f"https://calendarific.com/api/v2/holidays?api_key={key}&country=US&year={year}"
    )
    ...
    for item in fulljson["response"]["holidays"]:
        if "National" in item["type"][0]:
            ...
            cnt = (
                db.session.query(CalHolidays)
                .filter(
                    and_(
                        CalHolidays.holiday == newdate.holiday,
                        CalHolidays.date == newdate.date,
                    )
                )
                .count()
            )
...
def make_steps(step, dishid, i):
    newstep = Steps(dishfk=dishid, step_num=i, step_text=step)
...
def parse_results(fulljson):
    ...
    # weight
    if "serving_weight_grams" not in fulljson["foods"][0]:
        full_nutrition["weight"] = 0
    elif fulljson["foods"][0]["serving_weight_grams"] is None:
        full_nutrition["weight"] = 0
    else:
        full_nutrition["weight"] = round(
            fulljson["foods"][0]["serving_weight_grams"], 1
        )

    # calories
    if "nf_calories" not in fulljson["foods"][0]:
        full_nutrition["calories"] = 0
    elif fulljson["foods"][0]["nf_calories"] is None:
        full_nutrition["calories"] = 0
    else:
        full_nutrition["calories"] = round(fulljson["foods"][0]["nf_calories"], 1)

    # calories from fat
    if "nf_total_fat" not in fulljson["foods"][0]:
        full_nutrition["calFat"] = 0
    elif fulljson["foods"][0]["nf_total_fat"] is None:
        full_nutrition["calFat"] = 0
    else:
    ...
    # Saturated fat
    if "nf_saturated_fat" not in fulljson["foods"][0]:
        full_nutrition["satFat"] = 0
    elif fulljson["foods"][0]["nf_saturated_fat"] is None:
        full_nutrition["satFat"] = 0
    else:
        full_nutrition["satFat"] = round(fulljson["foods"][0]["nf_saturated_fat"], 1)

    # trans fat
    if "nf_trans_fatty_acid" not in fulljson["foods"][0]:
        full_nutrition["transFat"] = 0
    elif fulljson["foods"][0]["nf_trans_fatty_acid"] is None:
        full_nutrition["cholesterol"] = 0
    else:
        full_nutrition["transFat"] = round(
            fulljson["foods"][0]["nf_trans_fatty_acid"], 1
        )

    # cholesterol
    if "nf_cholesterol" not in fulljson["foods"][0]:
        full_nutrition["cholesterol"] = 0
    elif fulljson["foods"][0]["nf_cholesterol"] is None:
        full_nutrition["cholesterol"] = 0
    else:
        full_nutrition["cholesterol"] = round(fulljson["foods"][0]["nf_cholesterol"], 1)

    # sodium
    if "nf_sodium" not in fulljson["foods"][0]:
        full_nutrition["sodium"] = 0
    elif fulljson["foods"][0]["nf_sodium"] is None:
        full_nutrition["sodium"] = 0
    else:
        full_nutrition["sodium"] = round(fulljson["foods"][0]["nf_sodium"], 1)

    # Total Carbs
    if "nf_total_carbohydrate" not in fulljson["foods"][0]:
        full_nutrition["totalCarb"] = 0
    elif fulljson["foods"][0]["nf_total_carbohydrate"] is None:
        full_nutrition["totalCarb"] = 0
    else:
        full_nutrition["totalCarb"] = round(
            fulljson["foods"][0]["nf_total_carbohydrate"], 1
        )

    # Fiber
    if "nf_dietary_fiber" not in fulljson["foods"][0]:
        full_nutrition["fiber"] = 0
    elif fulljson["foods"][0]["nf_dietary_fiber"] is None:
        full_nutrition["fiber"] = 0
    else:
        full_nutrition["fiber"] = round(fulljson["foods"][0]["nf_dietary_fiber"], 1)

    # sugar
    if "nf_sugars" not in fulljson["foods"][0]:
        full_nutrition["sugar"] = 0
    elif fulljson["foods"][0]["nf_sugars"] is None:
        full_nutrition["sugar"] = 0
    else:
        full_nutrition["sugar"] = round(fulljson["foods"][0]["nf_sugars"], 1)

    # protein
    if "nf_protein" not in fulljson["foods"][0]:
        full_nutrition["protein"] = 0
    elif fulljson["foods"][0]["nf_protein"] is None:
        full_nutrition["protein"] = 0
    else:
        full_nutrition["protein"] = round(fulljson["foods"][0]["nf_protein"], 1)

    # potassium
    if "nf_potassium" not in fulljson["foods"][0]:
        full_nutrition["potassium"] = 0
    elif fulljson["foods"][0]["nf_potassium"] is None:
        full_nutrition["potassium"] = 0
    else:
