...
def get_mileage(origin_address, destination_address):
    key = (
        db.session.query(APIKeys.key)
        .filter(APIKeys.name == "googledistance")
        .first()
        .key
    )
    url = (
        f"https://maps.googleapis.com/maps/api/distancematrix/json?origins={origin_address}&destinations={destination_address}&mode=auto&language=en&units=imperial&key="
        + key
    )
    ...
    miles = re.findall(
        "\d*\.?\d+", fulljson["rows"][0]["elements"][0]["distance"]["text"]
    )
    return float(miles[0])


def add_oneway_reservation(
    date,
    time,
    tripnum,
    origin,
    ...
    price = int(mileage) * float(
        db.session.query(Mileage_catagory).filter_by(id=catagory).first().pricepermile
    )
    ...
    originalres = Mileage(
        ...
        time=datetime.time(
            datetime.datetime.strptime(time, "%H:%M").hour,
            datetime.datetime.strptime(time, "%H:%M").minute,
        ),
        catagoryfk=catagory,
...
def add_threeway_reservation(
    ...
    price = int(mileage) * float(
        db.session.query(Mileage_catagory).filter_by(id=catagory2).first().pricepermile
    )
    middleleg = Mileage(
        addressoriginfk=get_address(dest),
        originfk=dest,
        ...
        date=datetime.datetime.strptime(date, "%Y-%m-%d"),
        time=datetime.time(
            datetime.datetime.strptime(time2, "%H:%M").hour,
            datetime.datetime.strptime(time2, "%H:%M").minute,
        ),
...
def set_flag(id):
    ...
    if (
        updateflag.needTripNumber == True
...
def edit_catagory(id, catagoryname, description, pricepermile, imgcode):
    record = db.session.query(Mileage_catagory).filter_by(id=id).first()
...
def updatecost():
    miles = Mileage.query.all()
    
    for mile in miles:
        ppm = Mileage_catagory.query.filter_by(id = mile.catagoryfk).first().pricepermile
        ml = mile.mileage
        mile.tripprice = ml * ppm
        db.session.commit()