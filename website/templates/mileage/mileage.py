...
def mileageHome():
    if request.method == 'POST':
        ...
        if request.form.get('catagorybtn') == "catagory":
            ...
            add_doctor(**formdata)
            flash(formdata['firstname'] +" " + formdata['lastname'] + " was added to the database!", 'success')
    ...
    trips = db.session.query(Mileage).filter_by(userid = flask_login.current_user.id).order_by(desc(Mileage.date), Mileage.time ,Mileage.legnumber).all()
    ...
    doctors = db.session.query(Doctor).filter_by(userid = flask_login.current_user.id).all()
    facilities = db.session.query(Facility).filter_by(userid = flask_login.current_user.id).all()
    catagories = db.session.query(MileageCatagory).filter_by(userid = flask_login.current_user.id).all()
    tosendCount = db.session.query(Mileage).filter_by(tosend=True).count()
    needTripNumCount = db.session.query(Mileage).filter(Mileage.needTripNumber == True).filter_by(userid = flask_login.current_user.id).count()
    return render_template('/mileage/mileage.html', user=User, doctors=doctors, facilities=facilities, trips=trips, catagories=catagories,total=total, ntn=ntn, final=final, print=print, sent=sent, tosend=tosend, tosendCount=tosendCount, needTripNumCount=needTripNumCount)

@mileage.route('/fillpdf', methods=['GET', 'POST'])
@login_required
def fillpdf():
    numtrips = db.session.query(Mileage).filter_by(tosend = True).order_by(desc(Mileage.date), Mileage.legnumber).count()
    trips = db.session.query(Mileage).filter_by(tosend = True).order_by(desc(Mileage.date), Mileage.legnumber).all()
    ...
    inc = 0
    while pg <= pages:
        isExist = os.path.exists(os.path.join('website', 'static', 'pdf', datetime.datetime.now().strftime("%m%d%y")))
        if not isExist:
            os.makedirs(os.path.join('website', 'static', 'pdf', datetime.datetime.now().strftime("%m%d%y")))
        
        data = pdf_generate_datafile(smalltriplist[inc])
        pathTemplate = os.path.join('website', 'static', 'pdf', 'MileageReimburse.pdf')
        pathOutput = os.path.join('website', 'static', 'pdf', datetime.datetime.now().strftime("%m%d%y"), f'{pg}.pdf')
...
def tripnumadd(id):
    ...
    if request.method == "POST":
        ...
        legs = [float(baselegnumber+ .00), float(baselegnumber +.1), float(baselegnumber +.11)]
...
@login_required
def mileageTripNum():
    trips = db.session.query(Mileage).filter_by(userid = flask_login.current_user.id).filter_by(needTripNumber = True).order_by(desc(Mileage.date), Mileage.time ,Mileage.legnumber).all()
    if len(trips) == 0:
        return redirect(url_for('mileage.mileageHome'))
    doctors = db.session.query(Doctor).filter_by(userid = flask_login.current_user.id).all()
    facilities = db.session.query(Facility).filter_by(userid = flask_login.current_user.id).all()
    catagories = db.session.query(MileageCatagory).filter_by(userid = flask_login.current_user.id).all()
    tosendCount = db.session.query(Mileage).filter_by(tosend=True).count()
    return render_template('/mileage/tripnum.html', user=User, doctors=doctors, facilities=facilities, trips=trips, catagories=catagories,tosendCount=tosendCount)